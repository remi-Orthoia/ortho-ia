...
import {
  PREDIMEM_EXTRACT_PROMPT,
  PREDIMEM_EXTRACT_TOOL,
  type PredimemExtracted,
} from '@/lib/prompts/extraction/predimem'
...
/**
 * POST /api/extract-predimem-pdf — extracteur PDF dedie PREDIMEM.
 *
 * Specifique a PREDIMEM (Duchene & Jaillard, HappyNeuron 2019). 11 epreuves
 * memoire / 5 zones HappyNeuron / stratification age × NSC obligatoire.
 *
 * Pattern identique aux autres extracteurs dedies (EVALEO, Exalang 5-8 / 3-6
 * / 8-11 / Lyfac). PDF strict, max 10 Mo, Claude Sonnet 4.6 + tool_use force,
  ...
  try {
    ...
    if (kind === 'docx' || kind === 'image') {
      return NextResponse.json(
        { error: "PREDIMEM n'accepte que le PDF a l'import. Convertissez votre document en PDF avant import." },
        { status: 400 },
      )
    }
    if (kind !== 'pdf') {
    ...
    const content: Anthropic.MessageParam['content'] = [
      ...
      { type: 'text', text: PREDIMEM_EXTRACT_PROMPT, cache_control: { type: 'ephemeral' as const } },
    ]
        ...
        () => anthropic.messages.create(
          {
            ...
            max_tokens: 8192,
            tools: [PREDIMEM_EXTRACT_TOOL],
            tool_choice: { type: 'tool', name: 'extract_predimem_results' },
    ...
    if (!toolUse || toolUse.name !== 'extract_predimem_results') {
      logger.error('extract-predimem-pdf', new Error('Reponse Claude sans tool_use attendu'))
      return NextResponse.json({ error: 'Reponse IA invalide — reessayez.' }, { status: 502 })
    }

    const extracted = toolUse.input as PredimemExtracted
    return NextResponse.json({
      success: true,
      extracted,
    ...
    const handled = handleAnthropicError(err, "l'import du PDF PREDIMEM")
    if (handled) {
      logger.warn('extract-predimem-pdf-anthropic', err?.message ?? 'erreur Anthropic')
      return handled
    }
    logger.error('extract-predimem-pdf', err)
    return NextResponse.json(
