...
/**
 * Saisie structuree Exalang 11-15 (Helloin, Lenfant, Thibault — HappyNeuron 2009).
 *
 * Population : collège (6e à 3e). Outil de référence pour le suivi des
 * troubles des apprentissages au collège et le dépistage tardif d'élèves
 * non diagnostiqués au primaire (dyslexie compensée qui se révèle, trouble
 * de la compréhension écrite isolé, etc.).
 *
 * Scoring : percentile HappyNeuron (P5 / P10 / P25 / P50 / P75 / P90 / P95).
 * Étalonnage par niveau scolaire (6e / 5e / 4e / 3e).
 *
 * Groupes officiels HappyNeuron Exalang 11-15 (5 codes alphanumériques —
 * cohérent avec 8-11, structure stable) :
 *   - A.1 Langage oral
 ...
 *
 * Couplage : nouveau-crbo/page.tsx, test_utilise === ['Exalang 11-15'].
 *
 * Pattern aligné sur Exalang811ScoresInput.tsx. 16 épreuves officielles
 * (cf. lib/prompts/tests/exalang-11-15.ts).
 */
...
interface Props {
   ...
   *  encart d'évolution live + autodiagnostic du matching. */
...
/** 5 groupes officiels HappyNeuron Exalang 11-15 + 16 épreuves selon le
 *  manuel de passation et le module prompt `lib/prompts/tests/exalang-11-15.ts`.
 *  Les libellés DOIVENT rester strictement identiques à ceux du prompt pour
 *  permettre le matching IA en mode renouvellement. */
const GROUPES: Groupe[] = [
  {
    code: 'A.1',
    label: 'Langage oral',
    description: 'Fluences, compréhension orale de textes abstraits, dénomination rapide.',
    epreuves: [
      { key: 'fluence_phonemique',     label: 'Fluence phonémique (lettre, 1 min)',          hint: 'Profil dysexécutif si > sémantique. Analyser clusters et switches.' },
      { key: 'fluence_semantique',     label: 'Fluence sémantique (catégorie, 1 min)',       hint: 'Profil dysexécutif si < phonémique.' },
      { key: 'comp_orale_textes',      label: 'Compréhension orale de textes abstraits',     hint: 'Inférences, modèle mental du texte — préservée = trouble spécifique du langage écrit.' },
      { key: 'denomination_rapide',    label: 'Dénomination rapide de mots complexes',       hint: 'Accès lexical, manque du mot subclinique au collège.' },
    ],
  },
  {
    code: 'A.2',
    label: 'Métaphonologie / phonologie complexe',
    description: 'Conscience phonologique avancée — traces résiduelles d\'une dyslexie compensée.',
    epreuves: [
      { key: 'rep_logatomes',          label: 'Répétition de logatomes complexes',           hint: 'Boucle phonologique + représentations phonologiques. Sensible aux dyslexies résiduelles à l\'oral.' },
    ],
  },
  {
    code: 'B.1',
    label: 'Lecture',
    description: 'Voies d\'assemblage et d\'adressage, leximétrie texte long, compréhension inférentielle.',
    epreuves: [
      { key: 'lecture_mots_freq',      label: 'Lecture de mots fréquents',                    hint: 'Voie d\'adressage — doit être rapide et exacte au collège.' },
      ...
      { key: 'lecture_non_mots',       label: 'Lecture de non-mots / logatomes écrits',       hint: 'Voie d\'assemblage. Déficit = dyslexie phonologique (marqueur central, persistant à l\'ado).' },
      { key: 'leximetrie',             label: 'Leximétrie en contexte (texte long)',          hint: '6e : 140-180 mots/min ; 3e : 180-220 mots/min. Flag majeur si déficitaire.' },
      { key: 'comp_ecrite_inferentielle', label: 'Compréhension écrite inférentielle',        hint: 'Construction du modèle mental. Déficit isolé = trouble de la compréhension écrite.' },
    ],
  },
  {
    code: 'B.2',
    label: 'Orthographe / production écrite',
    description: 'Dictée, closure de texte complexe, production écrite narrative, raisonnement verbal.',
    epreuves: [
      { key: 'dictee',                 label: 'Dictée de mots, de phrases, de texte',         hint: 'Analyse qualitative : erreurs phonologiques / lexicales / grammaticales (homophones a/à, ses/ces).' },
      { key: 'closure_texte',          label: 'Closure de texte complexe',                    hint: 'Compréhension écrite + production. Mobilise inférences et cohérence narrative.' },
      { key: 'production_ecrite',      label: 'Production écrite narrative',                  hint: 'Cohérence du récit, densité lexicale, correction orthographique, complexité syntaxique.' },
      { key: 'raisonnement_verbal',    label: 'Raisonnement verbal (analogies, métaphores)',  hint: 'Compétences supra-lexicales — souvent atteintes dans les troubles de la compréhension.' },
    ],
  },
  {
    code: 'C.1',
    label: 'Mémoire et fonctions exécutives',
    description: 'Mémoire de travail verbale — boucle phonologique + administrateur central.',
    epreuves: [
      { key: 'empan_endroit',          label: 'Empan auditif endroit',                        hint: 'Boucle phonologique. Au collège, seuil < 5 alerte.' },
      { key: 'empan_envers',           label: 'Empan auditif envers',                         hint: 'Administrateur central. Envers très inférieur à endroit = profil dysexécutif / TDAH.' },
    ],
  },
]
...
const NIVEAU_OPTIONS = [
  { key: '',    label: '— choisir —' },
  { key: '6e',  label: '6e (~11-12 ans)' },
  { key: '5e',  label: '5e (~12-13 ans)' },
  { key: '4e',  label: '4e (~13-14 ans)' },
  { key: '3e',  label: '3e (~14-15 ans)' },
] as const
...
export default function Exalang1115ScoresInput({
  notes,
  onNotesChange,
  onResultatsChange,
  onError,
  ...
  // Import PDF Exalang 11-15 — route /api/extract-exalang-11-15-pdf.
  // Pre-remplit l'etat du form depuis un rapport informatise HappyNeuron Pro
  // ou un scan du cahier de passation.
  ...
  async function handleImportFile(file: File) {
    ...
    try {
      ...
      fd.append('file', file)
      const res = await fetch('/api/extract-exalang-11-15-pdf', { method: 'POST', body: fd })
  ...
  /**
   * Live preview deltas — mode renouvellement. Pattern aligné sur Exalang 8-11
   * (cf. Exalang811ScoresInput.tsx). Matching épreuve actuelle ↔ précédente
   * par label lowercase+trim, seuil ±10 sur percentile_value.
   */
  ...
  useEffect(() => {
    ...
    const lines: string[] = []
    lines.push('=== Exalang 11-15 (Helloin, Lenfant, Thibault — HappyNeuron 2009) ===')
    if (state.niveau) {
    ...
    <div className="space-y-4">
      <div className="flex items-start gap-2 p-3 rounded-lg border border-indigo-200 bg-indigo-50">
        ...
        <div className="text-sm">
          <p className="font-semibold text-indigo-900">Saisie structurée Exalang 11-15 — 5 groupes officiels (A.1, A.2, B.1, B.2, C.1)</p>
          <p className="text-indigo-700 text-xs mt-0.5 leading-relaxed">
            Reportez la zone HappyNeuron lue sur la feuille de résultats. Étalonnage par niveau (6e, 5e, 4e, 3e).
            Cochez « non passée » pour exclure une épreuve. Q1 = P25 = NORMAL, jamais déficitaire.
            Au collège, surveiller particulièrement la leximétrie et la compréhension écrite inférentielle (marqueurs majeurs).
          </p>
        </div>
      </div>

      {/* Import PDF Exalang 11-15 — rapport HappyNeuron Pro ou scan cahier. */}
      <div className="rounded-lg border border-sky-200 bg-sky-50/60 p-3">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div className="flex items-start gap-2 min-w-0">
            <FileUp size={18} className="text-sky-700 shrink-0 mt-0.5" />
            <div className="text-sm min-w-0">
              <p className="font-semibold text-sky-900">Importer un document Exalang 11-15 (optionnel)</p>
        ...
        <div className="rounded-lg border border-teal-300 bg-gradient-to-br from-teal-50 to-emerald-50 p-3">
          ...
          {evolutionStats.nouvelles > evolutionStats.totalCompared && evolutionStats.totalCompared <= 2 && (
            <p className="text-[10px] text-amber-800 mt-1 leading-relaxed bg-amber-50 border border-amber-200 rounded px-1.5 py-0.5">
              ...
              Vérifiez que les titres d&apos;épreuves du bilan précédent correspondent aux libellés Exalang 11-15 officiels.
            </p>
          )}
          {evolutionStats.nouvelles > 0 && (
            <p className="text-[10px] text-gray-600 italic mt-1.5 leading-tight">
              ⓘ Si le bilan précédent vient d&apos;une autre batterie (Exalang 8-11 / EVALEO par ex. — typique au passage 6e), l&apos;IA fera la comparaison sémantique côté CRBO généré — le tableau d&apos;évolution Word sera complet même si certaines épreuves apparaissent ici comme «&nbsp;nouvelles&nbsp;».
            </p>
          )}
        </div>
      )}
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex items-start gap-2">
                            <textarea
                              ...
                              rows={2}
                              placeholder="Observation : stratégies, type d'erreurs, attitude, fatigabilité…"
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-4">
        <div className="flex items-center justify-between mb-1">
          <label className="text-sm font-medium text-gray-800">
            Notes globales sur la séance (comportement, fatigabilité, stratégies, contexte scolaire)
          </label>
        ...
        <textarea
          ...
          rows={3}
          placeholder="Ex. élève coopératif, fatigue notable sur la production écrite, décompensation observée à l'entrée 6e, demande PPS en cours…"
