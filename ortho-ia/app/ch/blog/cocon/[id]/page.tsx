...
export function generateMetadata({ params }: PageProps): Metadata {
  ...
  return {
    title: `${cocon.label} — Blog Ortho.ia Suisse`,
    alternates: { canonical: `${BASE_URL}/ch/blog/cocon/${cocon.id}` },
  }
}

export const dynamic = 'force-static'

export default function ChCoconPage({ params }: PageProps) {
  const cocon = getCocon(params.id as CoconId)
  if (!cocon) notFound()

  const posts = getCoconPosts(cocon.id, 'fr-CH')

  return (
    <>
      <Header />
      <CoconPageContent coconId={cocon.id} posts={posts} locale="ch" />
      <Footer />
    </>
  )
}
