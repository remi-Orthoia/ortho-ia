...
> **Télécharger la trame CRBO + tester Ortho.ia** → [Télécharger la trame CRBO + tester Ortho.ia](https://ortho-ia.com/demo)
...
> **Bon à savoir** : Le CRBO doit être signé (manuscritement ou électroniquement) et daté. Un document non signé n'a pas de valeur juridique. Selon la Fédération Nationale des Orthophonistes (FNO), la signature électronique qualifiée offre le niveau de preuve le plus élevé ([fno.fr](https://www.fno.fr)).

### CRBO initial vs CRBO de renouvellement : deux logiques différentes

Un bilan initial pose le diagnostic et justifie l'entrée en rééducation. Un bilan de renouvellement **démontre l'évolution du patient et justifie la poursuite** de la prise en charge. Les sections restent identiques, mais le contenu change : le renouvellement compare les scores initiaux aux scores actuels, documente les progrès ou l'absence de progrès, et réajuste le projet thérapeutique. Pour comprendre en détail ce qui change entre les deux types de bilans, consultez [CRBO initial vs renouvellement : ce qui change dans la structure et la rédaction](/crbo-initial-vs-renouvellement/).
...
> **Attention** : Omettre la date de rédaction ou ne pas mentionner le médecin prescripteur expose à un rejet en contrôle. Un en-tête générique sans identification de la praticienne (ADELI, RPPS) n'est pas conforme.
...
> **À éviter** : Recopier mot pour mot la prescription sans contextualisation clinique. Cela n'apporte aucune information supplémentaire au prescripteur.
...
> **À éviter** : Une anamnèse en trois lignes pour un bilan enfant est insuffisante. L'absence de mention de la vérification audiologique est une lacune critique — elle écarte le diagnostic différentiel le plus fréquent (déficit auditif méconnu). Ne mélangez jamais les faits rapportés par la famille avec vos propres observations cliniques (celles-ci figurent en section 4).
...
**Règle critique** : mentionner **toutes** les épreuves administrées, y compris celles dans la norme. Un résultat normal est une information clinique — il permet de cerner le profil et d'écarter des hypothèses diagnostiques. Citer un test sans donner les scores (« *EVALO 2-6* administrée » sans résultats) est inadmissible en contrôle. Ne citer que les épreuves déficitaires biaise l'image du profil.

Pour approfondir la structuration complète de cette section avec exemples de formulation, consultez [Structure type d'un CRBO : trame complète avec exemples de formulation](/structure-type-crbo/).

> **Attention** : Omettre l'interprétation des écarts-types (par exemple, ne noter que « -1,8 ET » sans préciser que cela correspond à un déficit modéré) laisse le prescripteur sans repère clinique exploitable.
...
> **À éviter** : Conclusion en creux, diagnostic flou non posé, absence d'écartement des diagnostics différentiels principaux. Un diagnostic flou fragilise le dossier en cas de contrôle et n'aide pas le prescripteur dans sa décision.
...
> **À éviter** : Projet identique copié-collé d'un patient à l'autre (repérable en contrôle), absence de fréquence précisée, objectifs formulés de façon trop générale (« améliorer le langage » ne dit rien).
...
1. **Anamnèse trop courte** : trois lignes pour un bilan enfant sont insuffisantes. Minimum attendu : contexte de développement, antécédents médicaux, situation scolaire, vérification auditive. Reformulation correcte : développer chaque axe en 3-4 phrases.

2. **Tests cités sans scores** : inadmissible en contrôle. Si on administre une épreuve, on cote et on rapporte le résultat. Reformulation correcte : tableau complet avec notes brutes, étalonnées, interprétation.
...
4. **Projet thérapeutique copié-collé** : repérable par la CPAM, fragilise le dossier. Reformulation correcte : personnaliser les objectifs, la fréquence, la durée en fonction du profil clinique réel du patient.

5. **CRBO non signé et/ou non daté** : document sans valeur juridique. Reformulation correcte : signer et dater systématiquement avant envoi.
...
Ortho.ia génère le CRBO à partir de la dictée vocale des observations (transcription temps réel via Whisper), extrait automatiquement les scores depuis les grilles d'étalonnage photographiées (OCR), et rédige les sections 4 à 6 en cohérence avec les données saisies. Le praticien garde la main sur le raisonnement clinique, l'outil prend en charge la mise en forme. En pratique, le temps de rédaction passe de 2 h à 15-20 minutes (relecture et ajustements après génération).

> **Télécharger la trame CRBO + tester Ortho.ia gratuitement** → [Télécharger la trame CRBO + tester Ortho.ia](https://ortho-ia.com/demo)

---

## Pour aller plus loin : les ressources du guide

| Vous voulez... | Ressource |
|---|---|
| Une trame complète avec exemples de formulation section par section | [Structure type d'un CRBO](/structure-type-crbo/) |
| Des modèles de CRBO selon la pathologie (dyslexie, langage oral, aphasie) | [Modèles de CRBO par pathologie](/crbo-dyslexie-exemple/) |
| Comprendre ce qui change entre un CRBO initial et un renouvellement | [CRBO initial vs renouvellement](/crbo-initial-vs-renouvellement/) |
...
Pas de nombre de pages réglementé. En pratique, entre 3 et 5 pages pour un bilan initial standard, selon la complexité du profil. L'objectif est la **précision**, pas l'exhaustivité. Un CRBO de 8 pages avec des scores et une conclusion floue vaut moins qu'un document de 3 pages précis et bien structuré. La densité varie selon la pathologie : un bilan de langage oral enfant est souvent plus dense qu'un bilan de fluence isolé.

### Faut-il envoyer le CRBO au patient ou seulement au médecin ?

Le CRBO doit être adressé au médecin prescripteur. Le patient (ou ses représentants légaux s'il est mineur) a un droit d'accès au dossier médical conformément au Code de la santé publique. En pratique, de nombreuses orthophonistes remettent une **synthèse lisible** au patient lors de la séance de restitution (document simplifié, sans scores bruts ni jargon). Ce n'est pas la même chose que le CRBO complet avec cotations, qui reste un document technique destiné au prescripteur.

### Dans quel délai faut-il rédiger le CRBO après le bilan ?

Aucun délai légal précisément fixé en orthophonie libérale. En pratique, le standard professionnel est de rédiger dans les **15 jours suivant la dernière séance de bilan**. Un CRBO rédigé longtemps après (1 mois, 2 mois) expose à des incohérences entre les observations notées et le document final, car la mémoire des détails cliniques s'estompe. La CPAM peut interroger la cohérence temporelle en cas de contrôle (date du bilan vs date de signature du CRBO).

### La signature électronique est-elle valide sur un CRBO ?

Oui. La signature électronique est juridiquement valide pour les documents professionnels de santé dès lors qu'elle identifie de façon certaine le signataire. En pratique, une signature manuscrite scannée apposée sur le PDF est acceptable. Une signature électronique qualifiée (règlement européen eIDAS) offre le niveau de preuve le plus élevé et est recommandée en cas de litige.

### Comment formuler le diagnostic dans le CRBO ?

Utiliser une **terminologie nosologique reconnue** (TDL, dyslexie-dysorthographie, bégaiement développemental, dysphagie) avec sa sévérité (léger, modéré, sévère) et ses axes touchés. Exemple : « TDL à prédominance expressive, de sévérité modérée, touchant la phonologie et la morphosyntaxe en production ». Éviter les formulations en creux (« fragilités langagières », « difficultés persistantes »). Si le diagnostic ne peut pas être posé à l'issue du bilan (profil atypique, données insuffisantes), l'écrire explicitement et justifier : « Diagnostic différé, réévaluation prévue dans 6 mois après investigation complémentaire en neuropsychologie. »
...
Depuis la suppression de la DAP, le CRBO initial porte seul la responsabilité du volume de soins prescrit. Un document vague ou incomplet fragilise le dossier en cas de contrôle a posteriori.

> **Télécharger la trame CRBO + tester Ortho.ia** → [Télécharger la trame CRBO + tester Ortho.ia](https://ortho-ia.com/demo)

---

> **Disclaimer** : Cet article est rédigé à des fins d'information professionnelle générale à destination des orthophonistes libéraux. Il ne se substitue pas aux textes réglementaires en vigueur ni aux recommandations de la Fédération Nationale des Orthophonistes (FNO). Les exemples de formulation présentés sont fictifs et à des fins pédagogiques uniquement. Chaque CRBO doit refléter fidèlement les données cliniques réelles du patient et le jugement clinique de l'orthophoniste qui l'a réalisé. En cas de doute sur la conformité d'un document, consultez votre syndicat professionnel ou le service déontologie de votre URPS.