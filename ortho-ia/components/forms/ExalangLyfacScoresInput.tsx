...
const DOMAINES: Domaine[] = [
  {
    id: 'memoire',
    label: 'Mémoire',
    description: 'Empan visuel + empan verbal (endroit + envers) — 3 épreuves officielles du manuel.',
    epreuves: [
      { key: 'empan_visuel',  label: 'Empan visuel',  hint: 'MdT visuo-spatiale. Standard manuel : moy 5.82 (σ 1.08)' },
      { key: 'empan_endroit', label: 'Empan endroit', hint: 'Boucle phonologique (MdT verbale). Standard manuel : moy 6.49 (σ 1.12). Empan < 5 = signal fragilité.' },
      { key: 'empan_envers',  label: 'Empan envers',  hint: 'Administrateur central. Standard manuel : moy 5.13 (σ 1.29). Delta endroit/envers ≥ 3 = signal trouble exécutif / TDAH à investiguer.' },
    ],
  },
  {
    id: 'langage_elabore',
    label: 'Langage élaboré',
    description: 'Flexibilité lexicale, repérage d\'anaphores, consignes orales, inférences — 4 épreuves officielles du manuel.',
    epreuves: [
      { key: 'flexibilite_lexicale', label: 'Flexibilité lexicale',      hint: 'Variation lexicale, synonymes, paraphrase. Standard manuel : score moy 15.03 (σ 2.22), temps moy 222.93s (σ 58.15)' },
      { key: 'anaphores',            label: 'Repérage d\'anaphores',     hint: 'Repérage des anaphores du mot "Vénus" (13 anaphores, score sur 47 pts). Compétence pragmatique discursive. Standard manuel : score moy 33.14 (σ 7.57), temps moy 132.61s (σ 35.96)' },
      { key: 'consignes_orales',     label: 'Consignes orales',          hint: 'Compréhension consignes complexes à l\'oral (13 phrases-consignes). Standard manuel : score moy 10.55 (σ 1.45), temps moy 302.78s (σ 68.76 — très dispersé)' },
      { key: 'inferences',           label: 'Inférences',                hint: 'Compréhension implicite / sous-entendu (13 questions sur micro-récits). Standard manuel : score moy 10.13 (σ 1.73), temps moy 238.03s (σ 63.66)' },
    ],
  },
  {
    id: 'lecture',
    label: 'Lecture',
    description: 'Lecture de mots / logatomes + leximétrie (vitesse) + compréhension texte + repérage — 5 épreuves officielles du manuel.',
    epreuves: [
      { key: 'lecture_mots',      label: 'Lecture de mots',          hint: '60 mots isolés présentés à l\'écran en 1 min. Standard manuel : moy 59.89 mots (σ 7.01, normalité 49-71 mots/min). À croiser avec leximétrie.' },
      { key: 'lecture_logatomes', label: 'Lecture de logatomes',     hint: 'Voie d\'assemblage / phonologique pure. 10 logatomes à voix haute. Score moy 9.15/10 (σ 0.78, saturation) — le TEMPS est plus discriminant (moy 22.35s σ 4.57 ; sujets pathologiques 35.85s).' },
      { key: 'leximetrie',        label: 'Leximétrie',               hint: '⚡ Vitesse de lecture en CONTEXTE (texte 175 mots, 60s max). MLC moy 173.5/175 (σ 3.26), NTP moy 53.47 (σ 7.63). Critère majeur tiers-temps CDAPH.' },
      { key: 'comp_texte',        label: 'Compréhension de texte',   hint: 'Compréhension narrative (nouvelle policière). Score moy 14.16/18 (σ 2.18), temps lecture silencieuse moy 182.03s (σ 37.14). Ratio nb retours/temps retours informatif sur stratégies.' },
      { key: 'reperage',          label: 'Repérage',                 hint: 'Recherche d\'info dans page internet / texte (apports du para-texte). Score moy 7.65/10 (σ 1.25), temps moy 156.19s (σ 63.92). Croiser score + temps + ratio.' },
    ],
  },
  {
    id: 'orthographe',
    label: 'Orthographe',
    description: 'Texte à choix multiple (détection) + complétion de phrases (production) — 2 épreuves officielles + synthèse globale agrégée.',
    epreuves: [
      { key: 'texte_choix_multiple', label: 'Texte à choix multiple',  hint: 'Détection d\'erreurs en reconnaissance (3 propositions par mot cible). 2 sous-scores : Lexique moy 8.77 (σ 1.77), Grammaire moy 11.21 (σ 1.78). Temps moy 236.18s (σ 59.80). Saisir le percentile du score le plus discriminant ou le score global.' },
      { key: 'completion_phrases',   label: 'Complétion de phrases',   hint: 'Production orthographique guidée. 2 sous-scores : Lexique sur 22 moy 17.79 (σ 2.45), Grammaire sur 26 moy 21.97 (σ 2.81). À cet âge la grammaire est plus robuste que le lexique.' },
      { key: 'synthese_ortho',       label: 'Synthèse orthographique', hint: 'Agrégation (pas une épreuve séparée). Orthographe lexicale moy 26.65 (σ 3.35), Orthographe grammaticale moy 33.84 (σ 3.54). Optionnel — si l\'ortho saisit ici un score global du module.' },
    ],
  },
]
